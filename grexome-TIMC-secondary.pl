...
require($config);
grexomeTIMCsec_config->import(qw(refGenome vepCacheFile vepPluginDataPath InteractomeFile UniProtFile ClusterFile canonicalFile fastTmpPath),
...
# step 5.2
$com .= " | python3 $RealBin/5.2_addInteractome.py --inSampleFile $samples --inUniProt ".&UniProtFile($RealBin)." --inCandidateFile $candidateGenes_Int --inCanonicalFile ".&canonicalFile()." --inInteractome ".&InteractomeFile($RealBin)." --inClusterFile ".&ClusterFile($RealBin)." ";
if ($debug) {
