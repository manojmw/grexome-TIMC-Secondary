...
# input Uniprot output File generated by the UniProt_parser.py
my $inuniprot;

# input Canonical Transcripts file (.gz or non .gz)
my $incanonical;

# input High-quality Human Interactome(.tsv) file produced by Build_Interactome.py
my $ininteractome;

# input Cluster output file produced by clustering methods 
my $incluster;
...
--inuniprot : [optional] Uniprot output File generated by the UniProt_parser.py - [For integrating Interactome data]
--incanonical : [optional] Canonical Transcripts file (.gz or non .gz) - [For integrating Interactome data]
--ininteractome : [optional] High-quality Human Interactome(.tsv) file produced by Build_Interactome.py - [For integrating Interactome data]
--incluster : [optional] Cluster output file produced by clustering methods (If required, processed by appropriate ProcessClusterFile_*.py script') - [For integrating Interactome data]
...
GetOptions ("samples=s" => \$samples,
            ...
            "infile=s" => \$inFile,
            "cnvs=s" => \$cnvs,
            "inuniprot=s" => \$inuniprot,
            "incanonical=s" => \$incanonical,
            "ininteractome=s" => \$ininteractome,
...
if ($inuniprot) {
    (-f $inuniprot) || die "E $0: the supplied uniprot file doesn't exist\n";
}

if ($incanonical) {
    (-f $incanonical) || die "E $0: the supplied canonical transcripts file doesn't exist\n";
}

if ($ininteractome) {
    (-f $ininteractome) || die "E $0: the supplied interactome file doesn't exist\n";
}

if ($incluster) {
    (-f $incluster) || die "E $0: the supplied interactome clustering file doesn't exist\n";
}
...
# step 5.2
$com .= " | python3 $RealBin/5.2_addInteractome.py --insamples $samples --inuniprot $inuniprot --incandidate $candidateGenes_Int --incanonical $incanonical --ininteractome $ininteractome --incluster $incluster ";
if ($debug) {
